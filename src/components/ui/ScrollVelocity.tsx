...
function ScrollVelocityRowImpl({
  ...
  useAnimationFrame((_, delta) => {
    if (reduced || !isInViewRef.current || !isPageVisibleRef.current || isHoveredRef.current) return;
    const dt = delta / 1000;
  ...
  return (
    <div
      ref={containerRef}
      // Pause-on-hover is for mice only. On a touch screen a tap synthesises
      // mouseenter but the matching mouseleave may never arrive, which left the
      // marquee frozen for the rest of the visit. Leave always clears.
      onPointerEnter={(e) => {
